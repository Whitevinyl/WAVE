...
// Returns a random whole number between min & max in either a positive or negative direction //

Tombola.prototype.move = function(min,max) {
    var sign = (Math.round(Math.random()) * 2) - 1;
    return this.range(min,max) * sign; // int
};

// Returns a random float number between min & max in either a positive or negative direction //

Tombola.prototype.moveFloat = function(min,max) {
    var sign = (Math.round(Math.random()) * 2) - 1;
    return this.rangeFloat(min,max) * sign; // float
};

// Returns an array populated with random whole number moves //

Tombola.prototype.MoveArray = function(min,max,length) {
    var a = [];
    for (var i=0; i<length; i++) {
        a.push(this.move(min,max));
    }
    return a; // int array
};

// Returns an array populated with random float number moves //

Tombola.prototype.MoveFloatArray = function(min,max,length) {
    var a = [];
    for (var i=0; i<length; i++) {
        a.push(this.moveFloat(min,max));
    }
    return a; // float array
};
