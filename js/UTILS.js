...
function ampToNormal(val) {
    return (val + 1) / 2;
}

function mapToArray(array, val) {
    return val * array.length;
}

function linearInterpolate(array, val) {
    var dif = val - Math.floor(val);
    var before = array[Math.floor(val)];
    var after = array[Math.ceil(val)];
    return before + ((after - before) * dif);
}
...
function angleFromVector1d(a,b) {
